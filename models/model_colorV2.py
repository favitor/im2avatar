...
def placeholder_inputs(batch_size, im_dim, vol_dim):
  img_pl = tf.compat.v1.placeholder(tf.float32, shape=(batch_size, im_dim, im_dim, 3))
  vol_clr_pl = tf.compat.v1.placeholder(tf.float32, shape=(batch_size, vol_dim, vol_dim, vol_dim, 3))
  vol_flow_pl = tf.compat.v1.placeholder(tf.float32, shape=(batch_size, vol_dim, vol_dim, vol_dim, 2))
...
def get_loss(regressed_clr, blended_clr, target_clr, pred_flow, target_flow):
  ...
  '''
  num_pos_clr = tf.shape(input=tf.compat.v1.where(target_clr > -0.5))[0] # int
  num_pos_clr = tf.cast(num_pos_clr, dtype=tf.float32)

  num_pos_flow = tf.shape(input=tf.compat.v1.where(target_flow > -0.5))[0] # int
  num_pos_flow = tf.cast(num_pos_flow, dtype=tf.float32)

  ###################################
  # calculate positive loss for regrssed color
  regressed_clr_vol_loss = tf.abs(regressed_clr - target_clr)
  regressed_clr_pos_loss = tf.compat.v1.where(target_clr > -0.5, regressed_clr_vol_loss, tf.zeros_like(regressed_clr_vol_loss)) 
  regressed_clr_pos_loss = tf.reduce_sum(input_tensor=regressed_clr_pos_loss) / num_pos_clr

  # calculate positive loss for soft_blended color
  blended_clr_vol_loss = tf.abs(blended_clr - target_clr)
  blended_clr_pos_loss = tf.compat.v1.where(target_clr > -0.5, blended_clr_vol_loss, tf.zeros_like(blended_clr_vol_loss)) 
  blended_clr_pos_loss = tf.reduce_sum(input_tensor=blended_clr_pos_loss) / num_pos_clr

  ##################################
  # Calculate positive loss for flow
  flow_vol_loss = tf.abs(pred_flow - target_flow)
  flow_pos_loss = tf.compat.v1.where(target_flow > -0.5, flow_vol_loss, tf.zeros_like(flow_vol_loss))
  flow_pos_loss = tf.reduce_sum(input_tensor=flow_pos_loss) / num_pos_flow

  tf.compat.v1.add_to_collection('losses', regressed_clr_pos_loss + blended_clr_pos_loss + flow_pos_loss)
