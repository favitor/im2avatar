...
def placeholder_inputs(batch_size, im_dim, vol_dim):
  ...
  vol_pl = tf.compat.v1.placeholder(tf.float32, shape=(batch_size, vol_dim, vol_dim, vol_dim, 1))
...
def get_MSFE_cross_entropy_loss(pred, target):
  ...
  # FPE
  fpe = tf.compat.v1.where(target < 0.5, cross_entropy_loss, tf.zeros_like(cross_entropy_loss))
  num_neg = tf.shape(input=tf.compat.v1.where(target < 0.5))[0] # int 
  num_neg = tf.cast(num_neg, dtype=tf.float32)
  fpe = tf.reduce_sum(input_tensor=fpe) / num_neg


  # FNE
  fne = tf.compat.v1.where(target > 0.5, cross_entropy_loss, tf.zeros_like(cross_entropy_loss))
  num_pos = tf.shape(input=tf.compat.v1.where(target > 0.5))[0] # int
  num_pos = tf.cast(num_pos, dtype=tf.float32)
  fne = tf.reduce_sum(input_tensor=fne) / num_pos
